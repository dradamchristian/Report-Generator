...
function routeDatasetId(text) {
  const t = (text || "").toLowerCase();
  // Oesophagus / OGJ keywords
  if (t.includes("oesoph") || t.includes("esoph") || t.includes("ogj") || t.includes("gastro-oes") || t.includes("gastroes")) {
    return "oesophagus_resection_rcpath_v3_microscopy";
  }
...
exports.handler = async (event) => {
  try {
    ...
    const chosenDatasetId = datasetId || routeDatasetId(text);
    const { schema, rules, template } = readDatasetFiles(chosenDatasetId);
